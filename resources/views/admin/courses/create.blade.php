...
@section('content')
    ...
    <form action="{{ route('admin.courses.store') }}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="card card-body space-y-4">
                    <x-form.input name="title" label="Judul Kelas" placeholder="Judul kelas" required />
                    <x-form.textarea name="description" label="Deskripsi" placeholder="Deskripsi kelas" :rows="4"
                        required />
                    <x-form.input name="overview_video" label="URL Video Overview (YouTube)"
                        placeholder="https://youtube.com/watch?v=..."
                        hint="Video ini bisa ditonton user sebelum membeli kelas" />
                    <x-form.input name="whatsapp_group" label="Link WhatsApp Group"
                        placeholder="https://chat.whatsapp.com/..." />
                </div>

                {{-- Tools --}}
                <div class="card p-5" x-data="{ tools: [{ name: '', icon: '', url: '' }] }">
                        ...
                        <template x-for="(tool, index) in tools" :key="index">
                            <div class="grid grid-cols-3 gap-2 items-center">
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <button type="button" @click="tools.splice(index, 1)" x-show="tools.length > 1"
            ...
            <div class="space-y-4">
                <div class="card card-body space-y-4">
                    ...
                    <x-form.select name="level" label="Level" :options="['beginner' => 'Beginner', 'intermediate' => 'Intermediate', 'advanced' => 'Advanced']" />
                    ...
                    <x-btn type="submit" class="w-full justify-center">Simpan Kelas</x-btn>
                </div>
            </div>
        </div>
    </form>
