        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <div class="flex-1 min-w-48">
                <div class="relative">
                    ...
                    <x-form.input name="search" placeholder="Cari judul kelas..." wire:model.live.debounce.300ms="search"
                        class="pl-8 w-full" />
                </div>
            </div>
            ...
            {{-- Filter Level --}}
            <x-form.select name="level" placeholder="Semua Level" wire:model.live="level" class="w-36"
                :options="[
                    'beginner' => 'Beginner',
            ...
            {{-- Reset --}}
            @if ($search || $status || $level || $category_id || $showTrashed)
            ...
            <div class="flex items-center gap-3 mt-3 pt-3 border-t border-white/5">
                ...
                @if ($showTrashed)
                    <x-btn variant="primary" size="sm" x-data
                        ...
                        confirmAction({
                            title: 'Restore Kelas',
                            message: '{{ count($selected) }} kelas akan dikembalikan. Lanjutkan?',
                    ...
                    <x-btn variant="destructive" size="sm" x-data
                        ...
                        confirmAction({
                            ...
                            message: '{{ count($selected) }} kelas akan dihapus permanen dan tidak bisa dikembalikan!',
    ...
    <div class="card">
            ...
            <table class="table">
                ...
                <tbody>
                    @forelse($courses as $course)
                        <tr>
                            <td>
                                <x-form.checkbox name="selected" value="{{ $course->id }}"
                                    ...
                                    <div class="w-10 h-7 bg-white/5 rounded overflow-hidden flex-shrink-0">
                                        ...
                                        @else
                                            <div class="w-full h-full flex items-center justify-center">
                                                <i class="fas fa-play-circle text-[10px] text-white/20"></i>
                                            </div>
                                        @endif
                                    </div>
                            ...
                            <td>
                                <span class="text-xs text-white/40">{{ $course->category->name ?? '-' }}</span>
                            </td>
                            <td>
                                <x-badge>{{ ucfirst($course->level) }}</x-badge>
                            </td>
                            <td>
                    ...
                    confirmAction({
                        title: 'Restore Kelas',
                        message: '{{ addslashes($course->title) }} akan dikembalikan. Lanjutkan?',
                        confirmText: 'Ya, Restore',
                        type: 'restore',
                    }).then((confirmed) => { if (confirmed) $wire.restore({{ $course->id }}) })
                ">
                                            <i class="fas fa-undo text-[10px]"></i> Restore
                                        </x-btn>
                                        <x-btn variant="destructive" size="sm" x-data
                    ...
                    confirmAction({
                        title: 'Hapus Permanen',
                        message: '{{ addslashes($course->title) }} akan dihapus permanen dan tidak bisa dikembalikan!',
                        confirmText: 'Ya, Hapus Permanen',
                        type: 'danger',
                    }).then((confirmed) => { if (confirmed) $wire.forceDelete({{ $course->id }}) })
                ">
                                            <i class="fas fa-trash text-[10px]"></i> Hapus Permanen
                                        </x-btn>
                                    </div>
                                @else
                                    <div class="flex items-center gap-2 justify-end">
                                        <x-btn href="{{ route('admin.courses.show', $course) }}" wire:navigate
                                            variant="ghost" size="sm">Kelola</x-btn>
                                        <x-btn href="{{ route('admin.courses.edit', $course) }}" wire:navigate
                    ...
                    confirmAction({
                        title: 'Hapus Kelas',
                        message: '{{ addslashes($course->title) }} akan dihapus. Lanjutkan?',
                        confirmText: 'Ya, Hapus',
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center py-12">
                                <i class="fas fa-play-circle text-2xl text-white/10 mb-2 block"></i>
                                <p class="text-xs text-white/20">
                                    {{ $search || $status || $level || $category_id || $showTrashed ? 'Tidak ada hasil' : 'Belum ada kelas' }}
                                </p>
                                @if ($search || $status || $level || $category_id || $showTrashed)
        ...
        <div class="card-footer flex items-center justify-between">
            <p class="text-xs text-white/30">
                Menampilkan {{ $courses->firstItem() ?? 0 }}–{{ $courses->lastItem() ?? 0 }}
                dari {{ $courses->total() }} kelas
            </p>
            {{ $courses->links() }}
        </div>
    </div>
